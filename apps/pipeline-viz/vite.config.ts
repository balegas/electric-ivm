...
// The visualizer is a thin web front attached to a running electric-ivm engine. It proxies `/engine/*`
// to the engine's control-plane HTTP (default local port; override with ELECTRIC_IVM_ENGINE_URL), so
// the browser never needs CORS and you just point it at whichever engine you want to inspect.
const ENGINE = process.env.ELECTRIC_IVM_ENGINE_URL ?? 'http://127.0.0.1:3000'
