// Playground node renderer — same structure as pipeline-viz's (@viz/nodes), with audience-facing
// tag names: ROUTER instead of the internal "family" jargon, LIVE QUERY for a shape's output (it
// ties the canvas to the welcome headline). The dbsp-circuit tags stay deliberately technical.

import { Handle, Position, type NodeProps } from '@xyflow/react'

import type { NodeKind, VizNodeData } from '@viz/build-graph'

const KIND_META: Record<NodeKind, { color: string; bg: string; tag: string }> = {
  // logical view
  table: { color: '#334155', bg: '#e2e8f0', tag: 'TABLE' },
  family: { color: '#0369a1', bg: '#e0f2fe', tag: 'ROUTER' },
  filter: { color: '#b45309', bg: '#fef3c7', tag: 'FILTER' },
  sqnode: { color: '#7e22ce', bg: '#f3e8ff', tag: 'SUBQUERY NODE' },
  shape: { color: '#166534', bg: '#dcfce7', tag: 'LIVE QUERY' },
  ...
  sink: { color: '#166534', bg: '#dcfce7', tag: 'SINK · live query' },
}
