defmodule Electric.Integration.ElectricLiteOraclePropertyTest do
  @moduledoc """
  The Electric oracle PROPERTY test, run against electric-lite's `/v1/shape` adapter. Reuses Electric's
  own `WhereClauseGenerator.shapes_gen/1` + `StandardSchema` (DDL/seed/mutation generators) and
  `OracleHarness.test_against_oracle/4` — only the sync server under test is swapped for electric-lite.
  ...
  setup do
    lite_dir =
      System.get_env("ELECTRIC_LITE_DIR") || Path.expand("../../../../../dbsp-ds", __DIR__)

    cmd =
      "cd #{lite_dir} && exec env ADAPTER_WAIT_TABLE=level_3_tags pnpm --filter @electric-lite/bench exec tsx src/electric-adapter.ts"
  ...
  @tag timeout: 600_000
  property "electric-lite converges with the Postgres oracle across generated shapes + mutations", ctx do
  ...
  defp read_line(port, prefix, timeout) do
    receive do
      ...
      {^port, {:exit_status, status}} ->
        flunk("electric-lite launcher exited early (status #{status}) while waiting for #{prefix}")
    after
      timeout -> flunk("timed out waiting for '#{prefix}' from electric-lite launcher")
    end
  end
